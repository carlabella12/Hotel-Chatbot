...
namespace HotelChatBot
{
    ...
    public class HotelChatBot
    {
        ...
        {
            Console.Clear();
            var customer = customers[username];  // Retrieve the customer by username
            ...
            // Show all bookings of the customer
            ...
            if (int.TryParse(Console.ReadLine(), out choice) && choice >= 1 && choice <= customer.Bookings.Count)
            {
                ...
                // Calculate refund based on nights stayed
                ...
                {
                    // Remove the canceled booking from the customer's bookings
                    customer.Bookings.RemoveAt(choice - 1);

                    // Save the updated bookings and adjust the rooms
                    SaveReservations();
                    SaveRooms();

                    // Log the cancellation and refund
                    Console.WriteLine("Booking successfully canceled.");
                    Console.WriteLine($"Refund Amount: ${refundAmount}");

                    // Optionally, save the cancellation details (log to file)
                ...
                foreach (var line in File.ReadAllLines(ReservationsFile))
                {
                    ...
                    if (parts.Length == 7) // Ensure the correct number of fields
                    {
                        var booking = new Booking
                        {
                            ...
                            RoomNumber = parts[6] // Load the room number
                        };
        ...
        private void Checkout(string username)
        {
            ...
            // Display the customer's reservations including room number
            Console.WriteLine("\nYour Reservations:");
            for (int i = 0; i < customer.Bookings.Count; i++)
            {
                var booking = customer.Bookings[i];
            ...
            if (int.TryParse(Console.ReadLine(), out int choice) && choice >= 1 && choice <= customer.Bookings.Count)
            {
                ...
                Console.WriteLine("------------------------------------------");
                Console.WriteLine($"Room Number    : {selectedBooking.RoomNumber}");  // Display room number
                ...
                // Remove the checked-out booking from the customer's bookings
                customer.Bookings.RemoveAt(choice - 1);

                // Update the reservations file to reflect active bookings only
                ...
                // Clear the console after checkout
                Console.WriteLine("\nPress Enter to continue...");
                Console.ReadLine();
                Console.Clear();
            }
        ...
        private void ChangeRoomDetails()
        {
            ...
            if (rooms.ContainsKey(roomType))
            {
                try
                {
                    ...
                    SaveRooms(); // Persist changes to file
                }
        ...
        private void BookRoom(string username)
        {
            ...
            if (rooms.ContainsKey(roomType) && rooms[roomType].Count > 0)
            {
                try
                {
                    ...
                    // Assign room number based on type and available count
                    ...
                    int roomIndex = rooms[roomType].Count; // Use current count for numbering
                    string roomNumber = $"{roomNumberPrefix}{roomIndex:D2}"; // Room number format
                    ...
                    if (Console.ReadLine().ToLower() == "yes")
                    {
                        ...
                        // Create booking object with room number
                        var booking = new Booking
                        {
                            RoomType = roomType,
                            Nights = nights,
                            ...
                            RoomNumber = roomNumber, // Assign room number here
                            CheckoutTime = DateTime.Now.AddDays(nights) // Calculate checkout time
                        };

                        // Add booking to customer's account
                        customers[username].Bookings.Add(booking);

                        // Save reservation and room details immediately
                        SaveReservations();
                        SaveRooms();
